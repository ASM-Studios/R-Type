    ...
    _isRunning(true)
{
    ...
    _window->setPosition({0, 0}); //TODO REMOVE
...
void GUI::WindowManager::readServer() {
    while (this->_isRunning) {
        ...
        network::socket::udp::ServerManager::getInstance().getServer().getSocket().wait(boost::asio::ip::udp::socket::wait_read);
    }
}
...
static void ping() {
    ...
    network::socket::udp::ServerManager::getInstance().getServer().send("127.0.0.1", 8080, RawRequest(typedQuery));
...
void GUI::WindowManager::run() {
    _gameLogic.start();
    std::thread thread(&GUI::WindowManager::readServer, this);
